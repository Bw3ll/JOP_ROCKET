...
**A:** Yes, it is a standalone tool that is static analysis, and no, it cannot be integrated with a debugger like Mona. There is an option within JOP ROCKET to search through DLLs that it can find, and it can search through those, although obviously most system Windows DLLs will not be usable due to mitigations.  You can have JOP ROCKET exclude or include based on mitigations. Because it is static analysis, although it can optionally search through DLLs, it can miss some that are not found in the IAT or whose locations cannot be easily determined by JOP ROCKET. That is an inherent limitation to static analysis. By default, JOP ROCKET will search just through the image itself, i.e. the .exe, not the DLLs, to save time. If you are trying to analyze a binary and look at non-system DLLs, then that application must be installed, and you should provide its path. If you are not concerned with DLLs, then you need not install the binary.

**Q: Can you comment on the following quote, by Daniele Barattieri di San Pietro? "JOP ROCKET is a tool published in 2019 by [Bri19], that claims to generate full JOP chains in a fully automated way. Upon better inspection, we can see that any real world use-case of its technique is composed by a mixed ROP and JOP chain, where only a little portion of the result is composed by pure JMP Gadget, meanwhile the rest is composed by Gadget that ends with RET instruction, needed for the setup of the JOP chain."**

**A:** Unfortunately, the above reflects a poor understanding of JOP. JOP uses the dispatcher gadget paradigm, which requires a dispatch table and dispatcher gadget. The address of the dispatcher gadget and the dispatch table must be loaded into their respective registers to set up JOP.  Whilst this could be accomplished via JOP, for purposes of compatibility, this was done via two ROP gadgets using pop. Thus, after this initialization, consisting of only two ROP gadgets, no additional ROP gadgets are used. The number of JOP gadgets used can vary from one to dozens, depending on the input provided by the user. The problem here is not with the tool, but with a misunderstanding of the tool and JOP. JOP ROCKET does indeed provide exploits suitable for real-world applications, if you provide the appropriate input. Indeed, this was used in an Advanced Software Exploitation course previously taught by the author, requiring students to use JOP ROCKET to create different JOP exploits. It is disappointing the above comments were made in academic research, as only a maximum of two ROP gadgets are used for each JOP chain. The only real-world JOP exploit we are aware of that is on Exploit_DB is by one us, Austin Babcock, and as he explained at Hack in the Box Amsterdam 2021, his exploit closely mirrors what was found by JOP ROCKET. In most practical cases, it does not really make a difference if you use two ROP gadgets for setup. Sometimes there may not be appropriate JOP gadgets for a binary, in a way that conforms to the dispatcher gadget paradigm, for JOP setup. We would rather JOP be feasible on every binary, potentially, then to limit it out of a desire to have 100% pure JOP. One of the biggest limitations with JOP, after all, is with insufficient JOP gadgets. Writing in 2024, there are not active updates being made to JOP ROCKET, and in any possible new version, which would be much more advanced and patterned after the much newer ROP ROCKET, it likely would have an option for pure JOP.
